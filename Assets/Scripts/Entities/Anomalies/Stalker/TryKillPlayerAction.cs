...
public partial class TryKillPlayerAction : Action
{
	...
	protected override Status OnStart()
	{
		...
		float _killDistance = Mathf.Max(0.1f, KillDistance.Value);
	...
	private bool tryCacheAgentReferences()
	{
		...
		lookPoint ??= cachedTransform.Find("HighLookPoint");
	...
	private bool tryCachePlayerReferences()
	{
		...
		if (ReferenceEquals(cachedPlayerObject, _playerObject)) { return cachedPlayerMovement; }
	...
	private void triggerKill()
	{
		...
		cachedPlayerMovement.lockPlayer(lookPoint ? lookPoint : cachedTransform, Mathf.Max(0.1f, LockDuration.Value));
	}
}